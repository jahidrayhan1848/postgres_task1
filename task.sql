...
INSERT INTO orders (customer_id, order_date, total_amount) VALUES 
    ...
    SELECT customer_id, count(customer_id),sum(total_amount) as total_spent from orders GROUP BY customer_id HAVING count(customer_id) > 2
...
INSERT INTO employees (employee_name, department_name, salary, hire_date) VALUES 
    ...
    -- select max(salary) from employees where department_name= upper('hr');
     SELECT * from employees where salary > ( select max(salary) from employees where department_name= upper('hr'))

     SELECT *, (SELECT sum(salary) from employees) from employees;

     SELECT * from (SELECT department_name,salary from employees);
